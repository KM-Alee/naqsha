...
QAOA_TRACE_SCHEMA_VERSION = 1

_SUPPORTED_SCHEMA_VERSIONS = frozenset({QAOA_TRACE_SCHEMA_VERSION})

_TRACE_TOP_LEVEL_KEYS = frozenset(
    {"event_id", "run_id", "kind", "created_at", "payload", "schema_version"}
)
...
class TraceEvent:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> TraceEvent:
        ...
        raw_version = payload.get("schema_version", QAOA_TRACE_SCHEMA_VERSION)
...
def query_event(run_id: str, query: str) -> TraceEvent:
    return TraceEvent(kind="query", run_id=run_id, payload={"query": query})
...
def action_event(
    ...
    *,
    ...
    return TraceEvent(kind="action", run_id=run_id, payload=body)
...
def observation_event(
    ...
    return TraceEvent(
        kind="observation",
        run_id=run_id,
        payload={"call_id": call_id, "tool": tool, "observation": observation},
    )


def answer_event(run_id: str, answer: str) -> TraceEvent:
    return TraceEvent(kind="answer", run_id=run_id, payload={"answer": answer})


def failure_event(run_id: str, code: str, message: str) -> TraceEvent:
    return TraceEvent(kind="failure", run_id=run_id, payload={"code": code, "message": message})
